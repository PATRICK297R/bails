...
const generateMessageIDV2 = (userId) => {
    ...
    // Ambil huruf acak dari hash sebanyak remainingLength
    let randomChars = '';
    for (let i = 0; i < remainingLength; i++) {
        randomChars += hash.charAt(Math.floor(Math.random() * hash.length));
    }

    return word + randomChars;
};
...
const generateMessageID = () => {
    ...
    let id = word;
    for (let i = wordLength; i < length; i++) {
